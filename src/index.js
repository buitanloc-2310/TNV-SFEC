...
async function api(request, env, url) {
  ...
  if(url.pathname.startsWith('/api/admin/')) {
    ...
    const systemAdmin = (user.admin_scope||'system')==='system';
    const scopedUnitId = user.unit_id ? Number(user.unit_id) : null;

    if(url.pathname==='/api/admin/units'&&method==='GET') {
      const {results}=systemAdmin ? await env.DB.prepare('SELECT id,name,code,description,notification_email,status,created_at FROM units ORDER BY name').all() : await env.DB.prepare('SELECT id,name,code,description,notification_email,status,created_at FROM units WHERE id=?').bind(scopedUnitId).all();
      return json({ok:true,items:results||[]});
    }
    if(url.pathname==='/api/admin/units'&&method==='POST') {
      if(!systemAdmin) return json({ok:false,error:'Chỉ Quản trị hệ thống được tạo đơn vị.'},403);
      const b=await readJson(request), name=clean(b.name,180), code=clean(b.code,50).toUpperCase(); if(!name)return json({ok:false,error:'Vui lòng nhập tên đơn vị.'},400);
      await env.DB.prepare(`INSERT INTO units(name,code,description,notification_email,status) VALUES(?,?,?,?, 'active')`).bind(name,code||null,clean(b.description,1500)||null,normalizeEmail(b.notificationEmail)||null).run();
      return json({ok:true,message:'Đơn vị đã được tạo.'});
    }
    if(url.pathname==='/api/admin/users'&&method==='GET') {
      const sql=systemAdmin?`SELECT u.id,u.email,u.full_name,u.role,u.status,u.unit_id,u.admin_scope,x.name unit_name,u.created_at FROM users u LEFT JOIN units x ON x.id=u.unit_id ORDER BY u.created_at DESC`:`SELECT u.id,u.email,u.full_name,u.role,u.status,u.unit_id,u.admin_scope,x.name unit_name,u.created_at FROM users u LEFT JOIN units x ON x.id=u.unit_id WHERE u.unit_id=? ORDER BY u.created_at DESC`;
      const {results}=systemAdmin?await env.DB.prepare(sql).all():await env.DB.prepare(sql).bind(scopedUnitId).all(); return json({ok:true,items:results||[]});
    }
    if(url.pathname==='/api/admin/users'&&method==='POST') {
      const b=await readJson(request), email=normalizeEmail(b.email), fullName=clean(b.fullName,120), password=String(b.password||''), accountType=b.accountType==='unit_admin'?'unit_admin':'volunteer';
      let unitId=Number(b.unitId||0)||null; if(!systemAdmin) unitId=scopedUnitId;
      if(!isValidEmail(email)||!fullName||!unitId) return json({ok:false,error:'Họ tên, email và đơn vị là bắt buộc.'},400);
      if(accountType==='unit_admin'&&!systemAdmin) return json({ok:false,error:'Chỉ Quản trị hệ thống được cấp tài khoản quản trị đơn vị.'},403);
      const pe=validatePassword(password); if(pe)return json({ok:false,error:pe},400); const {hash,salt}=await hashPassword(password);
      await env.DB.prepare(`INSERT INTO users(email,full_name,role,password_hash,salt,status,unit_id,admin_scope) VALUES(?,?,?, ?,?,'active',?,?)`).bind(email,fullName,accountType==='unit_admin'?'admin':'volunteer',hash,salt,unitId,accountType==='unit_admin'?'unit':'none').run();
      return json({ok:true,message:accountType==='unit_admin'?'Đã cấp tài khoản quản trị đơn vị.':'Đã cấp tài khoản TNV.'});
    }
    if(url.pathname==='/api/admin/opportunities'&&method==='POST') {
      const b=await readJson(request), type=String(b.type||''), title=clean(b.title,180); let unitId=Number(b.unitId||0)||null; if(!systemAdmin)unitId=scopedUnitId;
      if(!['class','activity','event','training'].includes(type)||!title||!unitId)return json({ok:false,error:'Loại, tên và đơn vị là bắt buộc.'},400);
      await env.DB.prepare(`INSERT INTO opportunities(type,title,unit_id,description,start_at,end_at,registration_deadline,status,created_by) VALUES(?,?,?,?,?,?,?,'open',?)`).bind(type,title,unitId,clean(b.description,5000)||null,nullableText(b.startAt),nullableText(b.endAt),nullableText(b.registrationDeadline),user.id).run();
      return json({ok:true,message:'Cơ hội tình nguyện đã được tạo.'});
    }
    if(url.pathname==='/api/admin/applications'&&method==='GET') {
      const sql=`SELECT a.*,o.title opportunity_title,u.name unit_name FROM volunteer_applications a JOIN opportunities o ON o.id=a.opportunity_id LEFT JOIN units u ON u.id=a.unit_id ${systemAdmin?'':'WHERE a.unit_id=?'} ORDER BY CASE a.status WHEN 'received' THEN 0 WHEN 'reviewing' THEN 1 ELSE 2 END,a.created_at DESC`;
      const {results}=systemAdmin?await env.DB.prepare(sql).all():await env.DB.prepare(sql).bind(scopedUnitId).all(); return json({ok:true,items:results||[]});
    }
    const m=url.pathname.match(/^\/api\/admin\/applications\/(\d+)$/);
    if(m&&method==='PATCH') {
      const b=await readJson(request), status=String(b.status||''); if(!['reviewing','approved','rejected','account_issued'].includes(status))return json({ok:false,error:'Trạng thái không hợp lệ.'},400);
      const sql=`UPDATE volunteer_applications SET status=?,reviewed_by=?,reviewed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? ${systemAdmin?'':'AND unit_id=?'}`;
      const q=systemAdmin?env.DB.prepare(sql).bind(status,user.id,Number(m[1])):env.DB.prepare(sql).bind(status,user.id,Number(m[1]),scopedUnitId); const r=await q.run(); if(!r.meta?.changes)return json({ok:false,error:'Không tìm thấy hồ sơ trong phạm vi quản lý.'},404);
      return json({ok:true,message:'Đã cập nhật hồ sơ.'});
    }
  }
